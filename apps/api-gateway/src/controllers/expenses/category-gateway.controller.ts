...
export class CategoryGatewayController {
    ...
    async create(@Body() dto: any, @Request() req: any) {
        return this.expensesClient.send('expenses.categories.create', {
            userId: req.user.id,
            dto,
        });
    }
    ...
    @ApiQuery({ name: 'type', required: false, description: 'Filter by INCOME or EXPENSE' })
    @ApiResponse({ status: HttpStatus.OK, description: 'List of categories' })
    async findAll(@Query('type') type: string, @Request() req: any) {
        return this.expensesClient.send('expenses.categories.findAll', {
            userId: req.user.id,
            type,
        });
    }
    ...
    @ApiQuery({ name: 'type', required: false, description: 'Filter by INCOME or EXPENSE' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Categories with transaction counts and totals' })
    async getCategoriesWithStats(@Query('type') type: string, @Request() req: any) {
        return this.expensesClient.send('expenses.categories.withStats', {
            userId: req.user.id,
            type,
        });
    }
    ...
    async update(
        ...
        return this.expensesClient.send('expenses.categories.update', {
            id,
            userId: req.user.id,
            dto,
        });
    ...
    async delete(@Param('id') id: string, @Request() req: any) {
        return this.expensesClient.send('expenses.categories.delete', {
            id,
            userId: req.user.id,
        });
    }
